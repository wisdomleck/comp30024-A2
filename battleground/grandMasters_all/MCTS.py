...
- change opening
    - do the RPS triangle formation
    - move order should be swing captures -> swing moves -> throw captures -> slides

- keep difference between our throws and opponent throws to be <= 1
- test weights vs normal mcts greedy moves
"""
...
class MCTSNode:
    ...
    def __init__ (self, board, player, parent = None, last_action = None):
        ...
        throwsgap = (abs(self.board.unthrown_lowers-self.board.unthrown_uppers) <= 1)
        self.simultaneous_moves = self.get_possible_moves_greedy(throwsgap)
